// same file as https://github.com/KxSystems/kdb/blob/master/c/c/k.h but reformat and add comments
#ifndef KX
#define KX
typedef char *S, C;         // symbol, char
typedef unsigned char G;    // byte
typedef short H;
typedef int I;
typedef long long J;
typedef float E;            // real
...
// ku                       -> create guid
// ktn(I type, J length)    -> create a list
// kpn                      -> create a char array from string of length n
...
#define kF(x)    ((F*)kG(x))
#define kS(x)    ((S*)kG(x))
#define kK(x)    ((K*)kG(x))    // access mixed list
...
extern V m9(V);     // Free up memory allocated for the thread‘s pool

#endif

extern J gc(J j);

// khpun    -> connect with credential, timeout
// khpu     -> connect with credential
// khp      -> connect without credential
// okx      -> verify serialized bytes
// ymd      -> create a q date
// dj       -> convert q date to yyyymmdd integer
extern I khpunc(S, I, S, I, I), khpun(const S, I, const S, I), khpu(const S, I, const S), khp(const S, I),
        okx(K), ymd(I, I, I), dj(I);

// r0       -> Decrement the object‘s reference count
// sd0      -> callback
// sd0x     -> callback and close
extern V r0(K), sd0(I), sd0x(I d, I f), kclose(I);

// sn       -> intern n chars from a string
// ss       -> intern a string
extern S sn(S, I), ss(S);

// ktj      -> create temporal type list
// xT       -> create table
// xD       -> create dictionary
// krr      -> create k error
// ja       -> append an atom to a list
// js       -> append a string to a list
// jk       -> append a k object to a list
// jv       -> append a k list to a list
extern K ee(K), ktj(I, J), ka(I), kb(I), kg(I), kh(I), ki(I), kj(J), ke(F), kf(F), kc(I), ks(S), kd(I), kz(F),
        kt(I), sd1(I, K(*)(I)), dl(V *f, J), m4(I), knk(I, ...), kp(S), ja(K *, V *), js(K *, S), jk(K *, K),
        jv(K *k, K), k(I, const S, ...), xT(K), xD(K, K), ktd(K), r1(K), krr(const S), orr(const S), dot(K, K),
        b9(I, K), d9(K), sslInfo(K x), vaknk(I, va_list), vak(I, const S, va_list), vi(K, UJ), m4(I);
